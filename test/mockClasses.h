...
class FakeArduinoEnvironment : public IArduinoWrapper {
 public:
  ...
  int readAnalogFrom(int pinNumber) { return pinNumber; }
  int readDigitalFrom(int pinNumber) { return pinNumber; }
  ...
  void setBit(byte &byteToBeChanged, int bitPosition) { (byteToBeChanged) |= (1UL << (bitPosition)); }
  void clearBit(byte &byteToBeChanged, int bitPosition) { (byteToBeChanged) &= ~(1UL << (bitPosition)); }
...
class TestArduinoEnvironment : public IArduinoWrapper {
 public:
  ...
  void DelegateToFake() {
    ON_CALL(*this, setBit).WillByDefault([this](byte &byteToBeChanged, int bitPosition) { return fakeEnv.setBit(byteToBeChanged, bitPosition); });
    ON_CALL(*this, clearBit).WillByDefault([this](byte &byteToBeChanged, int bitPosition) { return fakeEnv.clearBit(byteToBeChanged, bitPosition); });
  }
...
class STDNamespaceFunctions : public IStandardFunctions {
 public:
  long maxValue(long firstValue, long secondValue) { return std::max(firstValue, secondValue); }

  long minValue(long firstValue, long secondValue) { return std::min(firstValue, secondValue); }
  ...
  int nextRandomInt() { return std::rand(); }

  int nextRandomIntInBounds(int lower, int upper) { return lower + std::rand() % (upper - lower + 1); }
};
...
class DummyAccelerationRatioMapper : public IAccelerationRatioMapper {
 public:
  ...
  virtual int mapToInt(float accelerationRatio) { return (int)(accelerationRatio * 100); }
};
