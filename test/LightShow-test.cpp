...
TEST_F(LightShowTest, testThatTheSequentialLedActivationExecutionFirstTurnsOfAllLedsOnTheBoardAndThenTurnsOnAllLedsFromAGivenStartIndex_TestSpinningToTheLeftSide)
{
    EXPECT_CALL(ledHeartMock, toggleBrightness(Eq(EXPECTED_BRIGHTNESS_FACTOR))).Times(Exactly(15));
    EXPECT_CALL(arduinoEnvMock, delayFor(Eq(EXPECTED_DELAY_INTERVAL))).Times(Exactly(15));
...
TEST_F(LightShowTest, testThatTheSequentialLedActivationExecutionFirstTurnsOfAllLedsOnTheBoardAndThenTurnsOnAllLedsFromAGivenStartIndexAndTurnsTheFormerLedOfWhenTheOptionIsSet_TestSpinningLeft)
{
    EXPECT_CALL(ledHeartMock, toggleBrightness(Eq(EXPECTED_BRIGHTNESS_FACTOR))).Times(Exactly(5));
    EXPECT_CALL(arduinoEnvMock, delayFor(Eq(EXPECTED_DELAY_INTERVAL))).Times(Exactly(5));
...
TEST_F(LightShowTest, testThatTheRandomHeartBlinkExecutionFirstTurnsOffAllLedsAndThenTurnsThenOnRandomlyWithTheGivenMinimumCount_CheckThatAfterThatTheLedsAreTurnedOfAgain)
{
