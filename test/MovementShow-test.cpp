...
class MovementShowTest : public ::testing::Test
{
    ...
    protected:
        void SetUp() override 
        {
            movementShow = new MovementShow(&gyroscopeMock);
        }
...
TEST_F(MovementShowTest, testThatTheMovementShowDetectesTheAccerlerationVectorDifferenceByCalculatingTheDifferenceBetweenTheCurrentAccerlerationAndTheFormerOneAndPassesItToTheActionMapper) 
{
    ...
    EXPECT_CALL(gyroscopeMock, wakeUp()).Times(Exactly(1));
    // The current and former acceleration are initialized in the gyrosscope
    EXPECT_CALL(gyroscopeMock, measureAcceleration()).Times(Exactly(3))
...
TEST_F(MovementShowTest, testThatTheMovementShowStoresTheCurrentAccelerationToMapTheDifferenceOfItToTheNextAccelerationToTheGivenActionMapper) 
{
    ...
    EXPECT_CALL(gyroscopeMock, wakeUp()).Times(Exactly(1));
    // The current and former acceleration are initialized in the gyrosscope
    EXPECT_CALL(gyroscopeMock, measureAcceleration()).Times(Exactly(4))
                                                     // former and current acceleration gets detected during wake up
...
TEST_F(MovementShowTest, testThatTheRandomBlinkLedMovementShowMapsTheAcclerationRatioToTheLedBrightnessAndTurnsOnSomeLedsRandomlyWhenTheRawAccerlerationDifferenceValueLiesOverItsGivenThreshold) 
{
...
TEST_F(MovementShowTest, testThatTheRandomBlinkLedMovementShowMapsTheAcclerationRatioToTheLedBrightnessAndDoesNotPerformAnyOtherActionWhenTheThresholdOfTheRawAccVectorDifferenceLiesUnderTheThreshold) 
{
    auto ledBrightnessMapper = new LedBrightnessAccelerationRatioMapper(10, 10);
    auto randomBlinkLedActionMapper = new RandomLedBlinkMovementShow(&ledHeartMock, ledBrightnessMapper, &testArduinoEnv, 1000.0f);
