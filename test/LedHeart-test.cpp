...
TEST_F(LedHeartTest, testThatTheLedHeartWillTurnOnTheLedsOnTheHeartRandomlyWithTheGivenCountOnLedsTurnedOnAtLeast) 
{
    ...
    EXPECT_CALL(stdNamespaceFunctionsMock, nextRandomIntInBounds(atLeastTurnedOn, HEART_LED_COUNT)).Times(Exactly(1))
                                                                                                   .WillOnce(Return(atLeastTurnedOn + 2));
    // Next 4 times the randomly turned on leds will be distinguished randomly
    EXPECT_CALL(stdNamespaceFunctionsMock, nextRandomIntInBounds(0, HEART_LED_COUNT)).Times(Exactly(4))
                                                                                     .WillOnce(Return(1))
                                                                                     .WillOnce(Return(4))
                                                                                     .WillOnce(Return(10))
                                                                                     .WillOnce(Return(6));
...
TEST_F(LedHeartTest, testThatTheLedHeartWillTurnOnTheLedsOnTheHeartRandomlyWithTheGivenCountOnLedsTurnedAndAlsoTestThatItCalculatesRandomNumbersUntilTheyAreUnique) 
{
    int atLeastTurnedOn = 2;
    // First time the rand int is created, the count on turned on leds is determined
    EXPECT_CALL(stdNamespaceFunctionsMock, nextRandomIntInBounds(atLeastTurnedOn, HEART_LED_COUNT)).Times(Exactly(1))
                                                                                                   .WillOnce(Return(atLeastTurnedOn + 2));
    // Next 4 times the randomly turned on leds will be distinguished randomly
    EXPECT_CALL(stdNamespaceFunctionsMock, nextRandomIntInBounds(0, HEART_LED_COUNT)).Times(Exactly(6))
                                                                                     .WillOnce(Return(7))
                                                                                     .WillOnce(Return(4))
                                                                                     .WillOnce(Return(4))
                                                                                     .WillOnce(Return(4))
