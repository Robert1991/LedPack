...
class GyroscopeTest : public ::testing::Test {
 ...
 protected:
  void SetUp() override { gyroscope = Gyroscope(&wireMock); }
  void TearDown() override { Mock::VerifyAndClearExpectations(&wireMock); }
};
...
TEST_F(GyroscopeTest,
       testThatTheGyroscopeFirstRequestsTheMeasurementOfTheWireAndThenReadsTheNextThreeRegistersInOrderToDetermineTheAccelerationVector) {
  auto reference = AccelerationMeasurementVector(1, 2, 3);
  EXPECT_CALL(wireMock, requestMeasurement()).Times(Exactly(1));
  EXPECT_CALL(wireMock, readNextRegister())
      .Times(Exactly(3))
...
TEST_F(GyroscopeTest, testThatTheAccerlationVectorDifferenceDelegatesTheAccelerationDifferenceRatioToTheGivenAccelerationRatioMapper) {
  auto vector = AccelerationMeasurementVector(1, 1, 1);
  EXPECT_THAT(vector.euclideanDistanceTo(vector).mapAccelerationRatioTo(new DummyAccelerationRatioMapper()), Eq(0));
}

TEST_F(GyroscopeTest, testThatTheGivenRawInputIsOnlyOverTheThresholdWhenItIsGreaterThanTheDetectedThreshold) {
  EXPECT_THAT(AccelerationMeasurementVector(1, 1, 1).euclideanDistanceTo(AccelerationMeasurementVector(2, 2, 2)).overThreshold(sqrt(3.00f)),
              Eq(false));
}
...
TEST_F(GyroscopeTest, testThatTheVectorDifferenceIsNotOverTheThresholdWhenTheGivenThresholdIsGreaterThanTheRawVectorDifference) {
  EXPECT_THAT(AccelerationMeasurementVector(1, 1, 1).euclideanDistanceTo(AccelerationMeasurementVector(2, 2, 2)).overThreshold(sqrt(3)), Eq(false));
}
...
TEST_F(GyroscopeTest, testThatTheVectorDifferenceIsOverTheThresholdWhenTheGivenThresholdIsSmallerThanTheRawVectorDifference) {
  EXPECT_THAT(AccelerationMeasurementVector(1, 1, 1).euclideanDistanceTo(AccelerationMeasurementVector(2, 2, 2)).overThreshold(sqrt(2.00f)),
              Eq(true));
}