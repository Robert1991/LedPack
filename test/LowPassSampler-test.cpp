...
{
    EXPECT_CALL(microphoneMock, readAnalog()).Times(Exactly(3))
    ...
    EXPECT_CALL(lowPassFilterMock, get()).Times(Exactly(3))
                                         .WillOnce(Return(15))
                                         .WillOnce(Return(100))
                                         .WillOnce(Return(50));
...
{
    EXPECT_CALL(microphoneMock, readAnalog()).Times(Exactly(6))
                                             .WillRepeatedly(Return(11));
    EXPECT_CALL(lowPassFilterMock, put(Eq(11))).Times(Exactly(6));
    // Max peak is initialized with 0
    // Min peak is initialized with 1023
    EXPECT_CALL(lowPassFilterMock, get()).Times(Exactly(6))
                                         // First interation
                                         .WillOnce(Return(15))
                                         .WillOnce(Return(100))
                                         .WillOnce(Return(50))
...
{
    EXPECT_CALL(microphoneMock, readAnalog()).Times(Exactly(9))
                                             .WillRepeatedly(Return(11));
    EXPECT_CALL(lowPassFilterMock, put(Eq(11))).Times(Exactly(9));
    // Max peak is initialized with 0
    // Min peak is initialized with 1023
    EXPECT_CALL(lowPassFilterMock, get()).Times(Exactly(9))
                                         // First iteration
                                         .WillOnce(Return(15))
                                         .WillOnce(Return(100))
                                         .WillOnce(Return(50))
...
TEST_F(LowPassSamplerTest, testThatTheLowPassSamplerDetectsTheFilterFrequencyByReadingTheGivenSamplesFromTheMicrophoneWhileMeasuringTheTimeDifferenceForThat)
{
    int START_TIME_MILLIS = 1000;
    int ENDTIME_TIME_MILLIS = 2000;
    int SAMPLES = 10;

    EXPECT_CALL(microphoneMock, readAnalog()).Times(Exactly(SAMPLES))
                                             .WillRepeatedly(Return(11));
    EXPECT_CALL(lowPassFilterMock, put(Eq(11))).Times(Exactly(SAMPLES));
    EXPECT_CALL(lowPassFilterMock, get()).Times(Exactly(SAMPLES))
                                         .WillRepeatedly(Return(15));
    EXPECT_CALL(stdFunctionsMock, getCurrentMilliseconds()).Times(Exactly(2))
                                                           .WillOnce(Return(START_TIME_MILLIS))
                                                           .WillOnce(Return(ENDTIME_TIME_MILLIS));
    EXPECT_CALL(stdFunctionsMock, maxValue(_, _)).Times(Exactly(10))
                                                 .WillRepeatedly(Return(15));
    
    EXPECT_THAT(lowPassSamplerWithStdFunctionMock -> detectFilterFrequency(SAMPLES), 
                Eq((SAMPLES * 1000)/(ENDTIME_TIME_MILLIS - START_TIME_MILLIS)));
}