...
class MicrophoneTest : public ::testing::Test {
 ...
 protected:
  void SetUp() override { microphone = Microphone(&testArduinoEnv, MICROPHONE_ANALOG_PIN, MICROPHONE_DIGITAL_PIN); }
  void TearDown() override { Mock::VerifyAndClearExpectations(&testArduinoEnv); }
};
...
TEST_F(MicrophoneTest, testThatTheMicrophoneReadsTheAnalogPinAndPassesTheValueToItsClientWhenTheMicrophoneFrequenceIsRead) {
  EXPECT_CALL(testArduinoEnv, readAnalogFrom(MICROPHONE_ANALOG_PIN)).Times(Exactly(1)).WillOnce(Return(25));
...
TEST_F(MicrophoneTest, testThatTheMicrophoneReadsTheDigitalPinAndPassesTheValueToItsClientWhenTheDigitalSwitchOnTheMicrophoneIsRead) {
  EXPECT_CALL(testArduinoEnv, readDigitalFrom(MICROPHONE_DIGITAL_PIN)).Times(Exactly(1)).WillOnce(Return(0));
