...
void IntegerBasedLowPassFilter::put(int input) {
  ...
  last_index++;
  if(this->last_index == INTEGER_BASED_LOWPASSFILTER_TAP_NUM)
    this->last_index = 0;
}
...
float FloatBasedLowPassFilter::filter_taps[FLOAT_BASED_LOWPASSFILTER_TAP_NUM] = {
  0.010846807119614697,
  0.015208111089537261,
  0.008282448904887219,
  -0.017383433599185996,
  -0.052973604369240555,
  -0.07315266507531389,
  -0.048940825894655864,
  0.030948034234790128,
  0.14561333634964552,
  0.24762366755104587,
  0.2884380300115189,
  0.24762366755104587,
...
void FloatBasedLowPassFilter::put(int input) {
  ...
  last_index++;
  if(this->last_index == FLOAT_BASED_LOWPASSFILTER_TAP_NUM)
    this->last_index = 0;
}
